...
4. [VGCバトルシステム](#4-vgcバトルシステム)
5. [Phase 7: 探索最適化](#5-phase-7-探索最適化)
6. [Phase 3: 試合データベース](#6-phase-3-試合データベース)
7. [データフロー](#7-データフロー)
8. [判断ロジック](#8-判断ロジック)
9. [ファイル構成](#9-ファイル構成)
10. [論文との整合](#10-論文との整合)
...
### 4.1 VGCDamageCalculator

**ファイル**: `src/domain/services/vgc_damage_calculator.py`

**目的**: VGC (Lv50 ダブルバトル) に完全対応した精密ダメージ計算

#### ダメージ計算式 (Gen 5+)

```
基本ダメージ = ((2 * 50 / 5 + 2) * 威力 * 攻撃 / 防御 / 50 + 2) * 補正
```
...
| 補正 | 値 | 説明 |
|------|-----|------|
| STAB | 1.5x | タイプ一致 (てきおうりょく: 2.0x) |
| 全体技 | 0.75x | ダブルで2体以上にhit時 |
| 天候 (晴れ) | 炎+50%, 水-50% | ひでり発動時 |
| 天候 (雨) | 水+50%, 炎-50% | あめふらし発動時 |
| すなあらし | SpD+50% | 岩タイプのみ |
| ゆき | Def+50% | 氷タイプのみ (Gen9) |
| フィールド | 1.3x | 対応タイプの技 |
| 壁 (リフレクター/ひかりのかべ) | 0.5x | 物理/特殊それぞれ |
| 急所 | 1.5x | ランク無視 |
| いのちのたま | 1.3x | - |
| たつじんのおび | 1.2x | 効果抜群時のみ |
| こだわりハチマキ/メガネ | 1.5x | 攻撃/特攻上昇 |

#### 乱数計算

- 16段階 (85%-100%)
- 全組み合わせで1発/2発/3発KO確率を計算
- 表示: 「確定X発」または「乱数X発 (Y%)」

### 4.2 世代別ルールシステム

**ファイル**: `src/domain/models/generation_config.py`

| 世代 | 名前 | 特殊ギミック |
|------|------|-------------|
| Gen 6 | XY/ORAS | メガシンカ |
| Gen 7 | SM/USUM | Zワザ |
| Gen 8 | SwSh | ダイマックス (3ターン, HP2倍) |
| Gen 9 | SV | テラスタル |

### 4.3 PokemonRulesRepository

**ファイル**: `src/domain/services/pokemon_rules_repository.py`

**役割**: 全ルール関連データへの統合アクセス

```
タイプ相性 → type_chart.py
技効果 → move_effect_db.py
特性 → ability_db.py
種族値 → base_stats_db.py
アイテム → item_effects.py
世代ルール → generation_config.py
```

### 4.4 実装済み特性

```
火力系: ちからもち, ヨガパワー, はりきり, てきおうりょく, ちからずく
災い系: わざわいのつるぎ, うつわ, たま, きば (25%デバフ)
天候系: ひでり, あめふらし, すなおこし, ゆきふらし
フィールド系: エレキ, サイコ, グラス, ミスト
免疫系: ふゆう, もらいび, ちょすい, ひらいしん, etc.
```
