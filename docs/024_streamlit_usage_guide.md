# 🎮 Streamlit UI 使用ガイド（アーカイブ）

> **重要:** 2025-11-19 時点で Streamlit ベースの UI（`frontend/streamlit_app.py`）は廃止され、React（`frontend/web/`）が唯一のフロントエンド実装になりました。  
> このドキュメントは参考用に残しています。実際の UI は以下のコマンドで起動してください。
>
> ```bash
> cd frontend/web
> npm install
